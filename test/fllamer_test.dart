...
void main() {
  ...
  group('config validation', () {
    ...
    test('json schema helper supports fixed-order optional properties', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        ...
        'properties': <String, Object?>{
          'answer': <String, Object?>{'type': 'string'},
          'maybe': <String, Object?>{'type': 'string'},
        },
      });

      expect(grammar, contains(r'"\"answer\""'));
      expect(grammar, contains(r'"\"maybe\""'));
      expect(grammar, contains('|'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'object',
          'additionalProperties': false,
          'required': null,
          'properties': <String, Object?>{
      ...
      final cyclicSchema = <String, Object?>{
        'type': 'object',
        'additionalProperties': false,
        'properties': <String, Object?>{},
      };
      (cyclicSchema['properties']! as Map<String, Object?>)['self'] =
          cyclicSchema;
      expect(() => llamaJsonSchemaGrammar(cyclicSchema), throwsArgumentError);

      final rawNestedSchema = <Object?, Object?>{
        'type': 'object',
        'additionalProperties': false,
      };
      rawNestedSchema['properties'] = <String, Object?>{
        'self': rawNestedSchema,
      };
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'array',
          'items': rawNestedSchema,
        }),
        throwsArgumentError,
      );
    });

    test('json schema helper infers type from schema keywords', () {
      expect(
        llamaJsonSchemaGrammar(<String, Object?>{
          'additionalProperties': false,
        }),
      ...
      expect(
        ...
        contains('schema1 ("," ws schema1)*'),
      );
      expect(
        llamaJsonSchemaGrammar(<String, Object?>{'minLength': 2}),
        contains(r'char{2,}'),
      );
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
    ...
    test('json schema helper ignores metadata annotations', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        '\$schema': 'https://json-schema.org/draft/2020-12/schema',
        '\$id': 'urn:fllamer:test',
        '\$comment': 'ignored by grammar generation',
        'title': 'Answer',
      ...
      expect(grammar, contains(r'char{1,}'));
    });

    test('json schema helper supports primitive type unions', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': <Object?>['string', 'null'],
      });

      expect(grammar, contains('schema1 ::= string'));
      expect(grammar, contains(r'schema2 ::= "null" ws'));
      expect(grammar, contains('schema1 | schema2'));
      final nullableStringGrammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': <Object?>['string', 'null'],
        'minLength': 1,
      });
      expect(nullableStringGrammar, contains('schema1 ::='));
      expect(nullableStringGrammar, contains(r'char{1,}'));
      expect(nullableStringGrammar, contains(r'schema2 ::= "null" ws'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
    ...
    test('json schema helper supports typed map objects', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'object',
        'additionalProperties': <String, Object?>{'type': 'integer'},
      });

      expect(grammar, contains('string ":" ws schema1'));
      expect(grammar, contains('schema1 ::= integer'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'object',
          'additionalProperties': <String, Object?>{'type': 'string'},
          'properties': <String, Object?>{
            'fixed': <String, Object?>{'type': 'string'},
          },
        }),
        _throwsUnsupportedFeature,
      );
    ...
    test('json schema helper supports bounded map objects', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'object',
        'additionalProperties': <String, Object?>{'type': 'integer'},
        'minProperties': 1,
      ...
      expect(
        grammar,
        contains('string ":" ws schema1 "," ws string ":" ws schema1'),
      );
      expect(
        llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'object',
    ...
    test('json schema helper supports JSON const and enum literals', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'const': <String, Object?>{'kind': 'ready', 'count': 1},
      });

      expect(grammar, contains(r'{\"kind\":\"ready\",\"count\":1}'));
      final enumGrammar = llamaJsonSchemaGrammar(<String, Object?>{
        'enum': <Object?>[
          <Object?>['x', 2],
          null,
        ],
      });
      expect(enumGrammar, contains(r'[\"x\",2]'));
      expect(enumGrammar, contains(r'"null" ws'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'const': <Object?, Object?>{1: 'bad'},
        }),
        throwsArgumentError,
      );
      expect(
      ...
      cycle.add(cycle);
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{'const': cycle}),
        throwsArgumentError,
      );
    });

    test('json schema helper supports small finite integer ranges', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'integer',
        'minimum': -1,
        'maximum': 2,
      ...
      expect(grammar, contains(r'"-1" ws | "0" ws | "1" ws | "2" ws'));
      expect(
        llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'integer',
          'exclusiveMinimum': -1,
      ...
      expect(
        ...
        contains(r'"0" ws | "2" ws | "4" ws | "6" ws'),
      );
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'integer',
    ...
    test('json schema helper supports bounded array lengths', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'array',
        'minItems': 1,
        'maxItems': 2,
      ...
      expect(grammar, contains('schema1 "," ws schema1'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'array',
          'minItems': 3,
    ...
    test('json schema helper supports fixed tuple arrays', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'array',
        'prefixItems': <Object?>[
          <String, Object?>{'type': 'string'},
          <String, Object?>{'const': 7},
        ],
      });

      expect(grammar, contains('schema1 "," ws schema2'));
      expect(grammar, contains(r'"7" ws'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'array',
          'prefixItems': null,
    ...
    test('json schema helper supports string length bounds', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'string',
        'minLength': 1,
        'maxLength': 3,
      ...
      expect(grammar, contains(r'char{1,3}'));
      expect(
        llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'string',
          'minLength': 2,
    ...
    test('json schema helper supports selected string formats', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'type': 'array',
        'prefixItems': <Object?>[
          <String, Object?>{'format': 'date'},
          <String, Object?>{'type': 'string', 'format': 'time'},
          <String, Object?>{'format': 'date-time'},
          <String, Object?>{'format': 'uuid'},
        ],
      });

      expect(grammar, contains(r'schema1 ::= "\"" date "\"" ws'));
      expect(grammar, contains(r'schema2 ::= "\"" time "\"" ws'));
      expect(grammar, contains(r'schema3 ::= "\"" date-time "\"" ws'));
      expect(grammar, contains(r'schema4 ::= "\"" uuid "\"" ws'));
      expect(grammar, contains('date ::='));
      expect(grammar, contains('uuid ::='));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
          'type': 'string',
          'format': 'email',
    ...
    test('json schema helper supports anyOf and oneOf alternatives', () {
      final grammar = llamaJsonSchemaGrammar(<String, Object?>{
        'anyOf': <Object?>[
          <String, Object?>{'type': 'string'},
          <String, Object?>{'const': 42},
        ],
      });

      expect(grammar, contains('root ::= schema0'));
      expect(grammar, contains('schema0 ::= schema1 | schema2'));
      expect(grammar, contains(r'"42" ws'));

      final oneOfGrammar = llamaJsonSchemaGrammar(<String, Object?>{
        'oneOf': <Object?>[
          <String, Object?>{'type': 'boolean'},
          <String, Object?>{'type': 'null'},
        ],
      });
      expect(oneOfGrammar, contains('schema0 ::= schema1 | schema2'));
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{'anyOf': <Object?>[]}),
        throwsArgumentError,
      );
      expect(
        () => llamaJsonSchemaGrammar(<String, Object?>{
